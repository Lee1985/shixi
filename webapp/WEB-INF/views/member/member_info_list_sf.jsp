...
// 		}
	...
	function doUpdateStatus(identityStatus) {
		if(identityStatus == '2' && $('#reply').textbox('getValue') == ''){
			$.messager.show({
					title : '提示',
					msg : '请填写反馈！'
				});
		...
		if (row.length==0) {
			...
			$.messager.confirm('Confirm', '你确定要执行操作吗?', function(r) {
				if (r) {
					...
					$.post('member/memberInfoUpdateIdentity.do', {
						ids : ids,
...
<body>
	...
	<div id="content" region="center" title="列表" style="padding:5px;width: 98%;height: 98%">
		...
		<div id="dlg_reply-buttons2">
			<a href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-ok" onclick="doUpdateIdentity('member/memberInfoUpdateIdentity.do','2')"
				style="width:90px">发送</a>
			<a href="javascript:void(0)" class="easyui-linkbutton"
