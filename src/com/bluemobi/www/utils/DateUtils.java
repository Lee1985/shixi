...
public class DateUtils {
	 ...
	 * @return
	 */
	public static long getDateSubtraction(Date minuend,Date meiosis) {
		if (logger.isDebugEnabled()) {
			logger.debug(">>>getDateSubtraction(Date minuend,Date meiosis)");
		}
		double day = 0;
		try {
			day =(double)(minuend.getTime()-meiosis.getTime())/(double)(24*60*60*1000); 
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (logger.isDebugEnabled()) {
			logger.debug("<<<getDateSubtraction() return day");
		}
		long lday;
		if(day > 0 && day < 1){
			lday = 1L;		
